...
// Função para aplicar o tema conforme o toggle
...
// Checar localStorage para tema salvo no carregamento
window.addEventListener('load', () => {
  const savedTheme = localStorage.getItem('loveAlbumTheme');
  const darkMode = savedTheme === 'dark';
  themeToggle.checked = darkMode;
  applyTheme(darkMode);
});

// Evento para trocar o tema e salvar a preferência
themeToggle.addEventListener('change', () => {
  const isDark = themeToggle.checked;
  applyTheme(isDark);
  localStorage.setItem('loveAlbumTheme', isDark ? 'dark' : 'light');
});